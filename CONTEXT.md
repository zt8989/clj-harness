...
**后台作业（job）** —— 一条**没人等**的命令：`bash_background` 起它、`bash_output` 读它打出来的
新行、`bash_kill` 停它并忘掉它。作业**没有时限**（跑到自己结束，或跑到被停），它记着自己的输出尾巴
（最近 500 行，丢掉的行数会说）与"本会话读到了哪一行"。**它只在进程里**：按会话分家、JVM 退出时
全部收掉、不落盘、不进库、**不跨重启**；也不随 run 结束而死——随 run 死就等于后台执行没用。
