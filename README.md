...
- `glob` 按**名字**找文件；`todo_write` 记本会话的任务清单（一次送**完整**清单，空数组即清空），
  **答案只报存了几项、各自什么状态**——清单就是刚送进去的那一份，不念回来；`todo_read` 是它的另一半，
  **没有参数**，把这份清单**读回来**（一项一行带状态标记，末尾一句总数与状态分布）。同理，`write` 的答案是
...
- `bash` 有 `timeout`（默认 120000ms，到点**连子孙一起**停）、`stdin`（写完随即关掉，读它的命令看到
  ...
  **不等的另一半是 `job`**：调用立刻返回，答案是 job id 与一句读法（`job_output`），命令在后台跑到自己结束
  （它没有时限，也不吃 `stdin`——两个键**不在它的参数表里**，传了也没有地方读）。
- 读它用 `job_output`（**最后一行**是状态，正文是它说过的最后一段；`offset` 从头翻，`wait: true` 挂到它结束，
  超时答 `[running]` 而不是报错；**装不下整份记录时答案里还写着那份记录在哪**，好让你 `grep` 或 `tail`
  它而不是一页页翻）；停它用 `job_kill`（不等到进程死透，停过之后**再问一次照样答**）；**回头找它用
  `job_list`**（没有参数，一行一份**记录**：本进程手里的作业、前面几次运行留下的、以及前台 `bash` 溢出
  落下的 `c*`——`j1` 在两棵进程戳下是两行，命令只有本进程还持有的那几行有，其余直说没有）。
  记录在 `<配置家>/jobs/<会话>/句柄-进程戳.log`，末行 `[exit N]` / `[stopped]`，**没有末行 = 还在跑**；
  名字里那截进程戳说的是「哪一次运行写的」，所以同一个会话重启之后不会写到上一次的记录上。**记录活过
  写它的那个进程**：JVM 退出只收走作业与句柄，文件留在配置家（昨天下班前那次长跑，今天还翻得到——路径
  不在回执里：`job` / `job_kill` 报的是 `job_output`，而它装不下整份记录时会把路径说出来）；整棵树按字节
  封顶，超了从最旧的一份开始删。
