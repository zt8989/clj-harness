...
### 命令等多久，以及不等人那种跑法（`timeout` / `job` / `job_kill`）
...
| 工具 | 一句话 |
|---|---|
| `job` | 起一条命令，立刻返回**句柄**（`j1`）与**那份记录在哪**。命令继续跑，输出**不在**这个答案里 |
| `job_kill` | 停掉整棵树并**忘掉**这条作业；记录留在原地，末行从此是 `[stopped]` |

**作业的记录是一份文件**：`<配置家>/jobs/<会话>/<句柄>.log`，一条命令一份。命令每打一行就追加一行、
flush 一行，所以读它用的是模型手里**已经有的**那几个读法——`bash`（`tail` / `grep` / `cat`）、`read`、
`grep`；配置家是围栏的自由路径（`cap.project/fence`），读它**不挂审批**。

**记录的末行是作业的状态**：命令走完、输出也排空了，末尾补一行 `[exit N]`；被本仓停掉补 `[stopped]`；
**没有这一行就是还在跑**——`exec 1>&-` 这种「流停了、进程还在」的写法就是没有这一行，本仓不替它编一个。
命令**自己把输出送走**（`… > 文件`）时，`job` 的答案会指名说一句：记录会是空的，输出在那个文件里；
只报告，不拦。

三件事都是**决定**，不是省略：

- **没有超时的是作业**。后台执行的全部意思就是没人等它；`timeout` 是「等多久」，两者不是一个旋钮。
- **没有人通知你它结束了**。没有推送通道，所以工具描述里写明了「你得自己来问」——而记录自己会说。
- **它只活在进程里**。按会话分家、JVM 退出时连记录一起收掉、不进库、也不随一轮 run 结束而死——
  随 run 死就等于后台执行没用。
...
侧边栏是三个段位：「新建任务」与「设置」钉在上下，**中间那一块是唯一滚动的东西**（窗口拉高拉矮
都不出现整页滚动），里面装着两块列表：

- **任务**在上面，平铺、不分组：**未绑定的会话**——不属于任何目录、也不记得任何目录的那些。
  「新建任务」造的就是它，**不需要先有项目**，而且不会因为界面上选中了某个项目就落进去。
  块为空时不画。
- **项目**在下面，就是原来那样：一个项目 = 一个目录，显示成它最后一个文件夹名；悬停出「更多」，
  里面有**移除项目**——那只是解绑，`projects/` 下的日志一个字节不动，重新添加同一目录会话就都回来
  （被移除项目放走的会话**不算任务**，它们记着那个目录，加回来就回家）。项目行右边的按钮在
  **这个项目里**开一场新会话，那是唯一一条「落在项目里」的路。
- **已归档**在最下面，默认折叠，只有一行，装的**既有任务也有项目的会话**（归档 = 行上的一个布尔，
  日志同样不动）；一行都没有时整块不画。项目会话那一行会写出它原来属于哪个项目——
  区块是平的，行上不说就没人说得出了。

会话的记录落在 `~/.clj-harness/projects/<workspace>/<threadId>.jsonl`；一个任务的记录落在
`projects/.unbound/` 下。
...
后端那条腿跑出来大概长这样（`main` @ `b7fe690`，基线随分支变，报数时带上分支与提交）：

```
Ran 895 tests containing 11493 assertions.
0 failures, 0 errors.
...
- 前端那条腿 44 个用例（`main` @ `b7fe690`），含 13 组：帧 schema / 真 `@ag-ui/client` 驱动 / 二轮续写 / 审批
  park→approve→veto / 技能列表（两层的根）/ 会话统计 / 上下文占用（那颗圈与它的面板）/ elicitation /
  界面取数 / 附件 / 一轮的折叠算术 / 侧栏那两行 / 两个会话同时跑。它自带后端（真 HTTP、真 `@ag-ui/client`，
