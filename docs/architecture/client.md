...
| 表 | 答什么 | 认得的名字 |
|---|---|---|
| `TOOL_ICONS` | **这是哪一只手**（kind，不是状态） | `read` `write` `edit` `replace` `insert` `undo_last_replace` `grep` `glob` `bash` `eval` `skill` `todo_write` `todo_read` `web_fetch` `web_search`；认不出的给 `WrenchIcon`，刻意不长得像其中任何一个 |
