...
`test/suites/{frames,client,turn,approval,skills,stats,context,elicitation,elicitation-card,attachments,turns,injections,picker,i18n,restore,concurrent,sidebar,session-title,relative-time,sidebar-rows,record,window}.ts`
是被它 import 的普通模块（`sidebar` / `record` / `window` / `elicitation-card` 那四份是 `.tsx`：它们
