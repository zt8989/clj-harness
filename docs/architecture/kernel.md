...
`.scratch/session-opening`），最后再接会话自己的派生注入（技能正文、作业结尾，见下）。**一轮 run 的输入是动作**：
...
- **后台作业的结束**（`harness.cap.jobs/before-llm`）：一条没人等的命令结束了，它的结局就作为一条
  `<job-ended id="…">[exit N]</job-ended>` 的尾随 user 消息摆在下一次调用面前——**两样事实
  （哪条作业、怎么结束的）加一句「用 `job_output` 读」，与记录多大无关**；**这不是推送**（不唤醒、不新起 run），
