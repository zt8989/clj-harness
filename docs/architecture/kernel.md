...
| `:context/injected` | 前置步骤刚放进历史的一条消息（指令块、人的 `/name` 要的正文、作业通知）——**模型被交给了它、它自己没要**。边把它发成 **`CUSTOM` 帧**，客户端画成一张卡，但**那不是对话的一部分**（只给屏幕看，见 [edge](edge.md)） |
...
drive! :
  ...
  loop:
    施加会话的前置步骤（cap.project/before-llm = 人的 `/name` 要的正文 + 作业结束的通知；见下）
...
`append`（**出生那一轮，开场块就在这个 `append` 里**：提问在前，指令文件与技能清单紧跟其后，
`.scratch/session-opening`），最后再接会话自己的派生注入（人的 `/name` 要的正文、作业结尾，见下）。**一轮 run 的输入是动作**：
...
（指令文件、清单，以及上一轮 `/name` 留下的正文、两次 run 之间结束的作业通知）是边先施加的，diff 也由它取、卡
...
**最后一列里与作业有关的那四个同属这一族**（`job` / `job_output` / `job_kill` / `job_list`——它们碰的是一条
