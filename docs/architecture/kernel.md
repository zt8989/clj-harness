...
| `:context/injected` | 前置步骤刚放进历史的一条消息（指令块、技能正文、作业通知）——**模型被交给了它、它自己没要**。边把它发成 **`CUSTOM` 帧**，客户端画成一张卡且**不回发**（见 [edge](edge.md)） |
...
`:context/injected` 事件，边把它转成 `CUSTOM` 帧（客户端画成一张卡，且**不回发**——见
...
**命令的记录落在配置家**（`<root>/jobs/<会话>/句柄.log`），不在会话的 jsonl 那棵树里：一份文件，
