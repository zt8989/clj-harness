...
——那次 run 由**服务端**把这场会话自己的消息、加上这次动作带的 `append`，按序交给
`harness.edge.ag-ui/first-user-text`，第一条 user 消息仍旧是第一句。代价写在
...
这也是它进库、而不是只留在对话里的理由：清单要活过一次 run（重启、另一个进程、将来某个面板读它），
而对话是**按记录重放出来的一段历史**——它能重建、能一段段读，却不是一张按 `thread_id` 查的表。
...
[ADR 0002](../adr/0002-sessions-live-on-the-server.md) 与 [overview](overview.md) 的铁律 3）：
权威在服务端内存里，记录是它的恢复源；重建交出去的是一份**副本**，服务端仍是那场会话的主人，
也仍然只有一条流式路径（run 的 SSE 与 `/feed`，后者不是第二条会话状态）。
...
**哪一轮没终结，按 run id 认，不按位置认**（2026-09-18）：日志里每条开启一个 run 的 `message` 行带它开启的 run id，
每条帧行带发出它的 run id——写侧一直守着这个配对（合上一轮时，补的帧就落在**那一轮**的 id 下）。所以「哪个 run 没结束」是 id 的问题。
按「最后一条开启行 / 最后一条终结帧」来问，在**一个 thread 同时有两个 run 在飞**时就不是答案：它们的行交错在同一份文件里，
**最后结束的那个 run 可以排在那个从未结束的 run 之后**。那次事故正是这个形状：三条 run 同时在跑，进程在中间被停，
一条更早的 run（`Cuqwlef`）没有终结帧，而最后一个开启的 run 正常收尾——拒绝侧数着「6 个开头对 5 个终结帧」
拒绝，修复侧问「最后那个 run 结束了吗」答「结束了」，于是**一个字的修复都不做**，这份日志永远读不出来。
...
一条开启 run 的 `message` 行后面没有终结帧，既可能是**运行中**，也可能是**进程被杀了**——两种留下的是同一份文件，
