...
- **后台执行同样不挂审批**：`bash` 的后台模式（`run_in_background`）与它的前台模式是同一个逃逸面
  （一条命令就是一条命令），`job_kill` 也一样，给谁单独挂个 park 都是装样子。后台模式的 cwd 与前台
  同一处解析——**记录也一样**：命令的输出写进配置家的 `<root>/jobs/<会话>/<句柄>-<进程戳>.log`，所以
  `read` / `grep` 读它**不 park**（配置家是围栏的自由路径）。这是一句陈述，不是一条新规：围栏一开始
  就把配置家列为自由。
