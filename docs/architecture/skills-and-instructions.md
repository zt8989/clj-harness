...
| **派生物**（加载时算一次，此后作历史） | 人的 `/name` 要的技能正文（`cap.skills/derived-injections`）、后台作业的结尾（`cap.jobs/before-llm`） | 首次那一轮折进对话（并作为卡留在会话里） | 加载是一次事件：正文进对话一次，此后 `model-view` 把卡里的字节还原给模型，`before-llm` 只是确认「已在」——既不重复追加，也不再重复画卡 |
...
人的 `/name` 要的技能正文由 `harness.cap.skills/derived-injections` 追加在最后——**只追加一次**：正文进对话后，
`sessions/model-view` 把它的卡还原回模型读到的那条消息，此后每轮它只是历史。散落在调用点就是把这个决定藏进两行 `into`，所以不许。
...
- **frontmatter 只读两个键**：`name` / `description`。其余（`allowed-tools` / `license` / `metadata` /
  `argument-hint` / `hidden` / `disable-model-invocation` …）**属于宿主，读到了也不报错**——一个文件多写了
  字段不是错误。解析是手写的窄解析器（扁平的 `key: value`、引号值、`|` 与 `>` 两种块标量），不是 YAML
  依赖：那会为了五个键买一个库，而它多接受的那些形状没人写过。
- **其中 `disable-model-invocation` 是明知其意而跳过的**，与 `allowed-tools` 同一条理由：**一份 SKILL.md
  说的是这个技能是什么，不是这场会话能拿它做什么。** 谁够得着一个技能只有两处说了算——`skill` 工具与
  人打的 `/name`（见「人也能加载」与「`skill` 工具」）——而一个文件若能关掉其中任一条，就是替会话回答了
  一个不属于它的问题。所以带这条字段的技能**照旧进清单、照旧能被工具加载**；跳过的字段原样留着，将来
  真认它的读者还读得到。
...
- **清单里没有的，只有坏技能**：`:available? false` 的（读不动、没有 frontmatter、`name` 与目录名不符、
  没有 description）不进清单——清单是给模型据以选择的，摆一份加载不动的东西，等于广告一件会话里没有的
  能力。它仍在 `scan` 里可见并注明原因，因为「文件在、能力没了、没人说得清为什么」是最坏的形态。
...
注入物于是成了历史的一部分。本仓的会话如今也归服务端了——**内存是权威、`jsonl` 是恢复源**——但注入物
仍然不是这场对话的一部分，理由换了：它是**每轮现算的派生文本，不是这场对话说过的话**。会话持有的是
**对话**（谁说了什么），而注入物每一轮都必须现读现拼：一份指令文件改一个字要下一轮就生效，一份技能在
...
| 谁在加载 | 怎么看出来 | 正文插在哪 |
|---|---|---|
| 模型 | **不经过这里**：`skill` 的结果就是正文本身，它是一条普通的工具结果 | 不追加（结果就是正文） |
...
- **人打的字一个字不改，`/name` 留在原地。** 这是选择不是疏忽：模型读到的「这个人说的话」就是他打的字，
  而正文作为**另一条消息**跟在后面。代价是一行上下文；换来的是**可重复读**——`/name` 是「这个技能被人
  要求加载」在对话里**唯一**的痕迹（`derived-injections` 正是扫它，见「技能正文的派生注入只剩人的那一半」），剥掉它就得
  另找一个地方记住「已经加载过」，而**人这条路的正文**本身是每轮现算的派生文本、不落进对话，那个地方不存在。
...
- **层跟着根走**：`harness.cap.skills/root-layers` 在**造出默认根的那一处**给它们贴上 `:system` / `:project`，
  ...
  **配置里 `:skills {:roots …}` 写的根没有层**——配置表里没有「系统 / 项目」这回事，从列表位置猜一个
  ...
  不是前端多一个分支。
- **列表与模型的清单只差一处：坏技能在人这份列表里**——带原因、但**不可选**（它不在可选条目里，所以
  方向键与 Enter 都够不到它）。
...
一个参数 `{"name"}`，成功时答的就是**那份 SKILL.md 的正文**（整份，一个字不截断），末尾跟一行说这个技能住在哪个目录：

```
<这份 SKILL.md 的全文，一个字不截断>

<技能目录> is the skill's directory; read files under it by absolute path.
```

- **末尾那一行目录不是装饰**：技能正文常写「读 `references/x.md`」，那是相对**技能目录**的路径，而这一行把目录说出来；围栏也正为这个根放行（见「与围栏的关系」）。
- **正文的落点就是这次调用的结果**：jsonl 里那条 `message` 行、`role` 是 `tool`，所以客户端**持有**它，会话栏里只有一张普通的工具卡——**没有第二张注入卡，也没有 `:context/injected` 事件**。多了一条代价：它是一条 `tool` 消息，`harness.edge.prune` 够得着它（超长正文在请求过压时可能被掐掉中段），要全文再调一次就有。
- 定位**只按清单里的名字查表，从不拼路径**。
...
- **两条路到达的是同一批技能**：`disable-model-invocation: true` 在这里**没有任何作用**（见「frontmatter
  只读两个键」）——文件说了不算，会话的两条路才说了算，所以它照旧进清单、照旧加载得动。两条路剩下的
  差别只在「没加载到」怎么报：这里是一句指名拒绝，人那条路是把同样内容的一句说明拼进对话
  （`harness.cap.skills/load-text`）。**两条路是两个 ask**：模型先加载、人再 `/name` 同一个名字，会追加第二份正文（见「技能正文的派生注入只剩人的那一半」）。
...
- 顶层仍是**整键替换**（与 `:approval` 同一条纪律），键**内部**才按键合并——`{:skills {:roots ["skills"]}}`
...
（见 [client](client.md#注入物在会话栏里的一张卡)）。

**那张卡是给屏幕的，对话里没有它。** 会话表确实留着它（屏幕要能重画它，重建也是从帧折出来的），而
`harness.edge.sessions/messages` 把 `data` part 从消息里摘掉——摘空了的那条整条不再出现，所以交给 run
的那份里没有注入的字节，模型下一轮收到的历史与从前逐字节相同。两半各有自己的守卫：服务端那半是
`harness.edge.sessions-test/the-cards-are-not-part-of-the-conversation`（注入的字节在模型那份里哪个字都
找不到），客户端那半是 `ui/test/suites/injections.ts` 第三条（适配器的出向转换只带 text / reasoning /
tool-call，`data` part 一个都不发；适配器升级时第一个要看的就是它）。
