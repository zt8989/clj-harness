...
| **派生物**（加载时算一次，此后作历史） | 技能正文（`cap.skills/derived-injections`）、后台作业的结尾（`cap.jobs/before-llm`） | 首次那一轮折进对话（并作为卡留在会话里） | 加载是一次事件：正文进对话一次，此后 `model-view` 把卡里的字节还原给模型，`before-llm` 只是确认「已在」——既不重复追加，也不再重复画卡 |
...
技能正文由 `harness.cap.skills/derived-injections` 追加在最后——**只追加一次**：正文进对话后，
...
会话中途从根里消失时要换成一句点名说明（见下）。于是正文每轮从会话自身重算，两条性质因此免费：
...
两条都不是「被记住的」：`/name` 是人打出来的字，tool call 与它的结果都在**会话**里，所以两条**下一次施加
...
- **人打的字一个字不改，`/name` 留在原地。** 这是选择不是疏忽：模型读到的「这个人说的话」就是他打的字，
  ...
  要求加载」在对话里**唯一**的痕迹（`derived-injections` 正是扫它，见「技能正文是派生的」），剥掉它就得
  另找一个地方记住「已经加载过」，而正文本身是每轮现算的派生文本、不落进对话，那个地方不存在。
