...
以及约定位置的**技能清单**；模型加载技能后，正文再补进对话。三件事由两个命名空间分管——
`harness.skills` 管技能（根、目录名即身份、frontmatter、坏技能诊断），`harness.preamble` 管开场那几条
user 消息的**顺序**与读指令文件的失败语义。
...
- **默认根**：`<OS 家目录>/.agents/skills`，绑定项目时再加 `<项目>/.agents/skills`。
  **前面的根赢同名**，所以项目里的一份同名技能盖过用户级的。
...
- **`disable-model-invocation: true` 的技能不进清单**：文件自己说这条不该由模型决定去用，而本会话
  没有别的入口。它仍在 `scan` 里可见并注明原因——「文件在、能力没了、没人说得清为什么」是最坏的形态。
...
点名说明（「技能 X 已不再在任何根里，它的指令读不到，也不该被假定」），**绝不静默少一段指令**。

判据只有一处：工具答的那句话的前缀（`harness.skills/loaded-prefix`）写与读**共用一份常量**。
被否决、被禁用、缺参数的调用根本到不了工具体，也就从不写下这句话——而在别处重建那个事实，
等于用第二个地方、从日志里重新推导「到底发生了什么」。
