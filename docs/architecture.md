...
- **界面说两种语言（i18n）**：计划见 `.scratch/ui-i18n/`（13 张票，2026-09-17 立），落地在
  worktree `.worktrees/ui-i18n`。**票 01（底座）已经落了，02–13 一张都没落**——机制、判定链、
  设置里那一行开关、以及视图切换那两条文案是真的（`docs/architecture/client.md` 的
  「文案与语言」一节写的就是它）；**其余每一面仍是硬写的英文**，一票一面地搬。
  它与「后端一个字节不翻」这条边界的关系见 client.md 的「边界」小节。
