(ns harness.system-prompt
  ...
  WHY THIS IS NOT PART OF harness.preamble, WHICH OWNS THE OTHER HALF. A run opens
  in two halves -- this one (role system) and harness.preamble's (the instruction
  ...
  is REQUIRE: the hooks below need the live tool table (harness.tools), the
  session's binding (harness.project) and the provider (harness.providers), while
  harness.project already requires harness.preamble for instruction-files -- so
  ...
  no longer true. This is the same discipline as config.edn, harness.edn,
  providers.edn and AGENTS.md, all of which are read fresh on every run. The price
  ...
  declared in harness.hooks -- that namespace owns the registry as a seam, and this
...
(defn assemble
  ...
  harness.http's set-up catch turns that into the RUN_ERROR a client sees. A
...
;; this namespace's docstring for the require cycle that decides it. harness.hooks
...
(defn- tools-block
  ...
  The set is read from the WIRE's array (harness.tools/specs) rather than from the
...
(defn- project-block
  ...
  (harness.project/binding-for ...)' was a true sentence about how to find out,
...
(defn- provider-block
  ...
  ONE SOURCE, NOT A NEW ONE. It reads harness.providers/active-provider, which is
  ...
  :api-key (harness.providers/active-provider), so there is no depth at which a
...
(defn- register-builtins!
  "Put the kernel's own rows at the SystemPrompt point. Called once, at load:
  register-builtin! replaces by name, so a namespace reload is a no-op rather than
  a second copy of each row."
  []
  (hooks/register-builtin! :system-prompt "tools" {:run tools-block})
  (hooks/register-builtin! :system-prompt "project" {:run project-block})
  (hooks/register-builtin! :system-prompt "provider" {:run provider-block}))
