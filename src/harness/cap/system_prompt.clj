(ns harness.cap.system-prompt
  ...
  hooks append is the part that is true of THIS session: which directory it is bound
  to, what this machine and its shell are. A fact written into
  ...
  is REQUIRE: the hooks below need the session's binding (harness.cap.project) and the
  machine (harness.infra.shell, behind harness.infra.env), while harness.cap.project
  already requires harness.cap.preamble for instruction-files -- so folding this half
  in there would be preamble -> project -> preamble, and triggering the point adds
  hooks.dispatch to the same loop (hooks -> project -> preamble). The other evidence
  sits on the preamble side: instruction-files takes (cfg, project-dir) instead of
  looking up the binding itself, precisely to dodge this cycle.
  ...
  IT ALSO REGISTERS THE KERNEL'S OWN HOOKS. Building one needs the namespaces
  above, which is exactly why they are registered from here rather than
...
(defn- env-block
  ...
  of that says whether this is Windows or POSIX, or whether `rg` is installed here.
  A model that writes `rg` on a machine without it gets one shell error per attempt,
  and that is the mistake this block exists to prevent -- which is why 'not found' is
  stated as plainly as 'available'.

  EVERYTHING IT SAYS IS READ, NEVER RE-DERIVED. The shell is harness.infra.shell's
...
(defn install!
  ...
  loaded is exactly what the install door exists to stop, and these two were the
  last ones doing it.

  The rows' IDs are `builtin:project` and friends, derived from the names below: a
