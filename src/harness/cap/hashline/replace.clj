(ns harness.cap.hashline.replace
  ...
  IT IS A PIPELINE, and this namespace is the order the steps have to happen in:
    ...
    1. parse the payload (harness.cap.hashline.edit) -- and fix or refuse the slips
  ...
  both reporting success -- see harness.cap.hashline.store, which explains why the
  ...
  (:require [clojure.data.json :as json]
            ...
            [harness.cap.hashline.insert :as insert]
...
(defn target-path
  ...
  harness.cap.hashline.store's ownership table guarantees -- so the file is DERIVED from
...
(defn- undo-record
  ...
  permission bits as well as the text -- see harness.cap.hashline.store and
...
;; CONCURRENTLY (`harness.kernel.loop/drive!`), and two edits to one file each validate
...
;;
;; WHO APPLIES IT is decided by `harness.kernel.tools/batch-plan`, which is where the
...
(defn- spans-of
  "The batch's edits as `harness.cap.hashline.anchors/align` reads them: ascending,
...
(defn perform-edits!
  ...
  in call order -- the tools pass one when the message had one; `harness.kernel.tools`
  ...
  (`harness.kernel.tools/turn-plan`), so a mismatch here would mean the grouping was wrong
...
(defn perform!
  ...
  (see harness.cap.editing). Returns a STRING -- what the model reads -- or throws a
  ...
  RESOLVE-PATH is `harness.cap.project/resolve-path` partialled on the thread in
...
;; ------------------------------------------------------------------- the batch
;;
;; SEVERAL ANCHOR EDITS TO ONE FILE IN ONE MESSAGE ARE ONE COMMIT, and the reason
;; is not tidiness -- it is that the alternative LOSES DATA. A turn's tool calls run
;; concurrently (`harness.kernel.loop/drive!`), and two edits to one file each
...
;; with BOTH reporting success. No lock around a single edit fixes that, because the
;; two edits are individually correct; what is wrong is applying them apart.
;;
;; So the edits of one message are applied as one patch: validated against the state
;; BEFORE the message, required to have disjoint line ranges (an overlap is a
...
;;
;; THE ARITHMETIC LIVES HERE, and the seam only ROUTES. `plan-turn` below answers
;; "which calls, if any, are one commit, and what is each call's part in it"; the
;; seam hands a turn to it once and then asks, per call, whether it is the appointed
;; one. Nothing in harness.kernel.tools knows what a `replace` is, which is why
;; these names are here now.

(defn- batchable?
  "May this call be merged with its siblings? `replace` and `insert` -- the two
  anchor-addressed edits whose ranges can be spliced into one patch at all."
  [name]
...
(defn plan-turn
  "The turn's calls as a plan: TOOL-CALL-ID -> its part in its message, for every
  call that shares its target file with another.
  ...
  EACH ENTRY CARRIES WHAT THE SEAM WOULD OTHERWISE HAVE TO ASK THIS NAMESPACE FOR,
  which is what makes the seam's routing three lines instead of an errand:

    {:role :applier|:member  :group n  :index i  :path p
     :run  (fn [] ...)   ; the appointed call -- runs the WHOLE group
     :note <sentence>}   ; a member -- its answer, having touched nothing

  THE APPOINTED CALL IS THE LAST OF THE GROUP, and it is the only one that writes.
  Members answer immediately and say where the outcome appears, so no call waits on
  another and a refusal or a park anywhere in the message cannot strand the rest."
  [thread-id calls]
  (let [parsed (keep (fn [{:keys [id function] :as call}]
                       (let [{:keys [name]} function]
                         (when (and (batchable? name)
                                    ...
                                    (contains? (tools/effective-tools thread-id) name))
    ...
    (into {}
          (for [[path g] groups
                :let [members (vec g)
                      n (count members)
                      args (mapv :args members)
                      run  (fn []
                             (perform-edits! thread-id
                                             #(project/resolve-path thread-id %)
                                             args
                                             (editing/editing-mode thread-id)))]]
            (into {} (map-indexed
                      (fn [i m]
                        (if (= i (dec n))
                          [(:id m) {:role :applier :group n :index i :path path :run run}]
                          [(:id m) {:role :member :group n :index i :path path
                                    :note (merged-note {:index i :size n :path path})}]))
                      members))))))
