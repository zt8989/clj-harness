(ns harness.cap.hashline.reading
...
(defn preview
  ...
  Returns:
    ...
    :shown        the anchors whose rows were EMITTED -- including an oversized
                  ...
                  edit may address; see harness.cap.hashline.store for why it is kept.
  ...
  (let [lines (mapv display-line (harness.cap.hashline.anchors/split-lines content))
        total (count lines)
