(ns harness.cap.hashline.store
  ...
  harness.infra.db/hashline-store) rather than by anything process-local.
  ...
  `harness.kernel.loop/drive!` runs a turn's tool calls CONCURRENTLY -- so two edits to
...
(defn claim-anchors!
  ...
  anchors it has established ARE this session's (harness.cap.hashline.replace's
...
(defn advance!
  ...
  CHANGE is `harness.cap.hashline.anchors/align`'s answer, plus the file's new
...
(defn restore!
  ...
  CHANGE is a `harness.cap.hashline.anchors/align`-shaped map whose `:anchors` are the
  ...
  current view to do it -- see harness.cap.hashline.undo, which holds the lock across
...
(defn with-session-lock
  ...
  And it happens: `harness.kernel.loop/drive!` runs a turn's tool calls concurrently, so
