...
(defn sessions
  ...
  :parent-id :subagent}, oldest first. `:archived?` is a BOOLEAN, converted here
  rather than left as the column's 0/1 -- Clojure's `boolean` says 0 is true, and a
  flag that means the opposite of what it looks like is the kind of bug that
  survives review.
  ...
  (mapv (fn [row] (-> row
                      ...
                      (dissoc :archived)))
        (db/select "SELECT id, project_id, path, archived, created_at, parent_id, subagent
