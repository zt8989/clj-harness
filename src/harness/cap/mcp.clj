        ...
        (fn [id msg]
          (let [params (:params msg)
                ...
                rec (when-let [{:keys [thread-id tool-call-id]} ctx]
                      (when-let [r (tools/parked-for-call thread-id tool-call-id)]
                        (when (= :elicitation (:reason r)) r)))
                decision (when rec (tools/take-decision! (:interrupt-id rec)))]
           ...
           (let [id (swap! next-id inc)
             ...
             (let [answer (deref p timeout-ms ::timeout)]
               ...
               (cond
                 ...
                 (:mcp-suspend answer)
                 (tools/suspend! (:thread-id ctx) (:tool-call-id ctx) (:mcp-suspend answer))
             ...
             :run (fn [args]
                    (let [result (try
                                   ...
                                   ((:request! client) "tools/call"
                                                       ...
                                                       {:thread-id thread-id
                                                        :tool-call-id tools/*tool-call-id*})
                                   (catch Exception e
...
(defn install!
  "Put this capability into the seam, and answer the TEARDOWN that withdraws it.

  TWO CONTRIBUTIONS, AND BOTH ARE ABOUT ONE SESSION AT A TIME, which is why the
  door takes functions rather than a map of definitions:

    :tools-for    which tools exist for this thread -- a server's roster depends
                  on which project's declaration is in force and on whether the
                  server is up, so it cannot be written down at setup.
    :disabled-for the names something switched off, with the sentence to say --
                  a switched-off server's tools stay in the table and their calls
                  are refused, and only this namespace knows which server that was
                  or how a person turns it back on.

  The seam learns nothing about servers from either one: it asks a question,
  gets tools or a sentence back, and relays it."
  []
  (tools/install! {:name "mcp"
                   :tools-for (fn [thread-id] (tools-for thread-id))
                   :disabled-for (fn [thread-id name]
                                   (when-let [server (server-of name)]
                                     (when (server-disabled? thread-id server)
                                       {:by "mcp"
                                        :message (str "disabled in this session: the MCP server "
                                                      (pr-str server)
                                                      " is switched off, so none of its tools"
                                                      " run -- " name " included. This is a"
                                                      " person's decision, not a missing"
                                                      " capability:"
                                                      " (harness.cap.mcp/session-enable-server!"
