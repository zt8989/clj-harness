(ns harness.infra.rg
  ...
  which is the same reasoning that put the shell lookup in harness.infra.shell.

  WHAT DOES NOT LIVE HERE. The `--json` event parsing stays in
  harness.cap.hashline.grep: a HIT is that namespace's idea, because a hit is a line
...
(defn- quoted
  "ARG as one single-quoted shell word. harness.infra.shell hands the whole line to
...
(defn run
  ...
  Returns harness.infra.shell/run's own map when rg finished: {:exit :out :err}.
