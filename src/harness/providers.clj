...
(defn- api-key
  ...
  The lookup itself is harness.home/env-value -- the same one the search vendor's
  key goes through, because 'a secret the person put outside the repository' is one
  kind of fact and two lookups would eventually disagree about precedence.

  PRIVATE, and doubly so by discipline: the key flows ONLY into
  resolve-provider's result, and prompt.md forbids reaching for it any other way."
  []
