(ns harness.kernel.hooks.dispatch
  ...
  THE CONTRACT, in three lines:
    ...
    other     per the point's :on-error -- :proceed for an observer, :block for a
              ...
              decide must not decide 'yes' (see harness.kernel.hooks/points).
  ...
  function the caller hands in, which is harness.edge.http's log! -- the only writer."
...
(defn- matches? [decl subject]
  (if-let [pattern (:matcher decl)]
    ;; The matcher was compiled at load (harness.kernel.hooks/check-declaration), so
...
(def ^:dynamic *sink*
  "The run currently firing hooks, or nil. Bound by the edge (harness.edge.http) for
...
(defn fire
  ...
  (harness.edge.http binds it), not something this returns."
