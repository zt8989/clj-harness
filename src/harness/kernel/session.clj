(ns harness.kernel.session
  ...
  a claim on the conversation -- taken as the entry is built and handed back as the entry is
  put away, so no other process serves a conversation this one is serving, and an idle
  conversation does not hold a claim forever either. That is ticket 04 of
  `.scratch/sessions-live-on-the-server`. THE CLAIM IS A SEAM NOW (tickets 08-10 of
  `.scratch/session-as-kernel`): `harness.cap.claims` implements it, and this namespace
  only asks. THE CLAIM'S TOKEN IS ALSO THE FEED'S
...
;; ------------------------------------------------------------------------- the seams
;;
;; WHAT THIS NAMESPACE OWNS, AND WHAT IT MERELY ASKS FOR. The MECHANISM is here: one walk over
;; a record, per-thread state, the two subscription seams, the window arithmetic, the run and
;; stop pins (tickets 08-10 of `.scratch/session-as-kernel`). WHAT A RECORD IS -- where a log
;; lives, how a line is parsed, how a line becomes a conversation, what a provider may be
;; handed -- is the ADAPTER's, and it arrives through `install!`:
;;
...
;; the read stream says there is no record. `harness.edge.sessions` installs the real ones.
;;
;; THE IRON LAW RIDES ON `:build`: the mechanism never reads a record during a run. The one
;; read is the walk a session is BORN by, and it is the adapter's.

(def ^:private default-seams
  {:build          (fn [_thread-id _folds]
...
(defn install!
  "Install the adapter's half of the seams: a map, merged over the defaults (see the block
  above). THE COMPOSITION ROOT CALLS THIS (`harness.edge.http/start!`); nothing here happens
  because a namespace was loaded, so 'why does this session have a real answer' is answered
  by the line that installed it rather than by what somebody else required."
  [m]
  (swap! seams merge m)
  nil)

(defn reset-seams!
  "Put the DEFAULTS back -- the teardown of an `install!`. For a test that must see the
  mechanism with nothing installed, and for a composition root that stops."
  [ ] ; no arguments
  (reset! seams default-seams)
  nil)
...
;; ------------------------------------------------------- the read stream's consumers

(defonce ^:private read-folds
  ;; name -> {:init (fn [] v) :step (fn [acc [line-index row]] acc)}. SEE `register-fold!`.
  ;; A consumer registers ONCE, at namespace load, and the value it produces for a session
  ;; is kept on that session under `:folds`. Nothing here re-reads a record.
  (atom {}))

(defn register-fold!
  "Register a consumer's FOLD on the session's READ stream (ticket 02): NAME -> {:init (fn []
  v) :step (fn [acc [line-index row]] acc)}.

  THE SESSION OWNS THE ONE WALK OVER THE RECORD, and a consumer does not make a second
  one: when a session is built, every registered fold is fed each row of the record, in
  file order, on that same walk, and its final value is kept ON the session -- read it back
  with `fold-value`. This is the door 'everything else is a consumer' comes through: a
  consumer registers a fold instead of opening the file and folding it itself.

  REGISTER AT NAMESPACE LOAD (`harness.edge.pressure` registers the meter's band this way).
  A session already in the table does not gain a fold until it is built again, because the
  fold's value IS the walk; registration is not a subscription to a running session."
  [name {:keys [init step]}]
  (swap! read-folds assoc name {:init init :step step})
  nil)

(defn unregister-fold!
  "Stop registering NAME's fold. FOR A TEST (and a report), never for the run path: a
  consumer registers once at load and does not take it back."
  [name]
  (swap! read-folds dissoc name)
  nil)

;; ----------------------------------------------------- the write stream's consumers

(defonce ^:private write-steps
  ;; name -> (fn [value ctx [line-index row]] value). SEE `register-step!`.
  ;; A live step advances the value its FOLD produced, one written row at a time.
  (atom {}))

(defn register-step!
  "Register a consumer's LIVE step on the session's WRITE stream (ticket 04): NAME -> a step
  of the SAME shape a fold uses -- `(fn [value ctx [line-index row]] value)` -- with no
  `:init`, because the value it advances is the one its fold produced at build.

  THE ONE WRITER PATH (`harness.edge.http/log!`) calls `row-written!` for every row it hands
  the writer, and every registered step advances IN PLACE -- so adding a consumer is
  registering a step, not editing `log!`. Register at namespace load, exactly as a fold is."
  [name step]
  (swap! write-steps assoc name step)
  nil)

(defn unregister-step!
  "Stop calling NAME's live step. FOR A TEST, never for the run path."
  [name]
  (swap! write-steps dissoc name)
  nil)
...
(defn row-written!
  ...
  fold, and there is no separate seed read to pay."
  [thread-id [i row]]
  (let [id (str thread-id)
        steps @write-steps]
    (when (and (seq steps) (contains? @registry id))
...
(defn registered-folds []
  "The read-stream folds registered right now, by name. FOR A READER -- a test, a report."
  @read-folds)
...
(defn- model-messages
  "ENTRIES + COMPACTIONS + PRUNES -> the conversation AS A PROVIDER MAY BE HANDED IT. THE
  RULE IS THE ADAPTER'S (`harness.edge.replay/compacted-messages` of `model-view`): what a
  card is, and how one is realised, is the wire's business rather than this mechanism's.
  The default seam hands the entries back unchanged."
  [entries compactions prunes]
  ((:model-messages @seams) entries compactions prunes))

(defn- build
  "THREAD-ID's conversation as the record has it: the entry a session is born with
  (`:entries` / `:compactions` / `:prunes` / `:context` / `:state` / `:folds`).

  THE WALK IS THE ADAPTER'S (`:build`), and it is handed the READ FOLDS registered here, so
  a consumer's fold rides the same walk the session's own fold does (ticket 02). An adapter
  that has no walk -- a session whose log is nowhere -- answers an empty conversation, which
  is the right answer for a thread that has never run."
  [thread-id]
  ((:build @seams) thread-id @read-folds))
...
(defn- ensure-session
  ...
  (if-some [e (get @registry thread-id)]
    e
    (let [claim  ((:take! (:claim @seams)) thread-id)
      ...
      (if-some [won (get before thread-id)]
        (do ((:hand-over! (:claim @seams)) thread-id (:token claim) (:claim won))
            (get after thread-id))
        (get after thread-id)))))
...
(defn fold-value
  "THREAD-ID's answer from the read-stream fold NAME (`register-fold!`), or nil when this
  process is not holding the session, or the fold was not registered when it was built.

  A LOOKUP, like `live-entry`: asking does not build a session and does not run the walk."
  [thread-id name]
...
(defn read-records
  ...
  rule, the tree walk and the torn-tail rule live here, not in each caller.

  Answers one of:
    {:ok records}        the record's rows, a torn LAST line dropped (`replay/read-records`);
    {:missing sentence}  nothing under the tree is named after THREAD-ID -- the locator's own
                         sentence, which a route answers 404 with;
    {:error sentence}    the record is there and cannot be read (400)."
  [thread-id]
  ((:read @seams) thread-id))

(defn fold-record
  "THREAD-ID's RECORD folded as a STREAM (tickets 05-07): INIT and RF, handed every row as
  `[line-index row]`, read through the session. A consumer that only folds holds only its
  answer. Answers the same three shapes as `read-records`, with `{:ok <the fold's answer>}`."
  [thread-id init rf]
  ((:fold @seams) thread-id init rf))
(defn generation
...
(defn messages
  ...
  (let [id (str thread-id)]
    ...
    (let [e (get @registry id)]
      (model-messages (:entries e) (:compactions e) (:prunes e)))))
  ...
  (into {}
        (map (fn [[tid e]]
               [tid {:entries    (count (:entries e))
                     :messages   (count (model-messages (:entries e) (:compactions e) []))
                     :runs       (:runs e)
                     :state      (:state e)
