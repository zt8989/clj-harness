(ns harness.hashline.insert
  ...
  not a promise bolted on. `harness.hashline.anchors/align` keeps an anchor when the
...
(defn parse
  ...
  The anchor and the lines go through `harness.hashline.edit`'s field parsers, so
