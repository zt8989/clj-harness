(ns harness.hashline.grep
  ...
  and the three refusals a spawned search can produce live in harness.rg, because
...
(defn run-rg
  ...
  The running is harness.rg's -- the executable, the timeout, and the three
  ...
  case that a model can act on is the refusal harness.rg raises, not this."
  [args dir]
...
(defn- anchor-rows
  ...
  harness.hashline.reading produces, and for the same reason: the caller has to know
