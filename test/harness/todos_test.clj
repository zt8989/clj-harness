(ns harness.todos-test
  ...
  IT IS REPLACED RATHER THAN APPENDED, which is what makes it state (harness.db
...
(defn- in-a-fresh-jvm
  ...
  harness.project-test. And a path crossing a process boundary belongs in an
  ...
  (let [dir (io/file (System/getProperty "java.io.tmpdir")
    ...
    (try
      ...
      (let [r (in-a-fresh-jvm
               (str "(require 'harness.todos)"
                    " (spit (System/getenv \"CLJ_HARNESS_TEST_OUT\")"
                    " (pr-str (harness.todos/items-for \"todos-test\"))"
                    " :encoding \"UTF-8\")")
               out)]
