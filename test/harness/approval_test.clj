...
(deftest reading-a-skills-reference-file-does-not-park
  ...
  ;;
  ;; THE SKILL GOES IN A HOME OF THIS TEST'S OWN (see support/with-temp-env): the
  ;; shared one belongs to every test in the JVM, and a planted skill left there is
  ;; an `alpha` that every later run's catalog lists.
  (support/with-temp-env
   [_root home]
   (let [proj      (support/temp-dir "approval-proj")
         skill-dir (str (io/file home ".agents" "skills" "alpha"))
         ref-file  (str (io/file skill-dir "references" "x.md"))
         elsewhere (support/temp-dir "approval-elsewhere")]
