(ns harness.hashline-store-test
  ...
  Every test here goes through `harness.db` on the test runner's own temp home,
...
(deftest a-store-with-the-home-tables-migrates-without-losing-a-row
  ...
  ;; store's path comes from harness.home. So it moves the root -- and puts back
  ...
  (let [step-1  [(first db/migrations)]
    ...
    (let [dir (io/file (System/getProperty "java.io.tmpdir")
      ...
      (try
        ...
        (is (= ["projects" "sessions"] (db/tables step-1))
...
(deftest one-file-is-edited-by-one-thread-at-a-time
  ;; The reason the lock exists: `harness.loop/drive!` runs a turn's tool calls
