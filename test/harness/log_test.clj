(ns harness.log-test
  ...
  line lands in `<root>/logs/harness.log`, and that the appender's policy really
...
(defmacro with-capture
  ...
  THE BINDING IS NOT DECORATION. `harness.log/error!` calls
  `harness.logging/ensure!`, which reconfigures whenever `harness.home/root` has
...
(deftest an-error-reaches-the-file-and-the-console-as-the-same-sentence
  (with-capture "both" root out
    ...
    (testing "with a real stack trace -- what handing the Throwable over buys"
      ...
      (is (str/includes? (str out) "harness.log_test")))))
...
(deftest logging-follows-the-root-when-the-root-moves
  ;; The reason `logging/ensure!` exists. A test binding moves harness.home/root,
