...
(deftest the-fence-is-on-the-tool
  ;; The marker is what makes an out-of-bounds root park for a human; the parking
  ;; itself is the seam's ordinary approval flow, tested where approvals are tested.
  (is (true? (:fence-paths (get (tools/effective-tools tid) "glob")))))
