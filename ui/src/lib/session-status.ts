...
export const archiveRefusal = (status: SessionStatus): string =>
  status.parked
    ? "That session is waiting on you; archive it once it settles."
    : "That session is still running; archive it once it settles.";
...
export const removeProjectRefusal = (
  ...
  status.parked
    ? `${sessionId} is waiting on you; remove the project once it settles.`
    : `${sessionId} is still running; remove the project once it settles.`;
