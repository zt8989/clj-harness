...
- ~~**不设「读」工具**：清单就在上一次调用的答案里，模型每次都送完整的，没有"读一半再改一半"这种用法。
  ...
  **2026-09-24 翻掉（`.scratch/readback-verbs/spec.md` 决策 2）**：那句理由只在一个上下文窗口内成立。
  清单之所以进库，正是因为它**跨 run、跨进程**活着——压缩过一轮、换过一个进程、或另一个调用写过之后，
  模型手里只剩「有一份」这个记忆，读不回来。于是有了 `todo_read`：**没有参数**（清单属于本会话），
  一项一行带状态标记，末尾一句总数与状态分布，渲染与 `items-for` 同处——读侧从此**也**有一个动词。
  面板那条结论（`.scratch/tool-parity/spec.md` 决策 14「不做 UI 面板 / HTTP 端点」）**不动**。
