...
11. **搜索只有一个厂商，键名固定。** `HARNESS_SEARCH_API_KEY`，解析顺序与 provider 的
...
12. **第二个搜索厂商、自定义端点：另一张票。** 本票把一家的线写在一处（`harness.web.search`，
    端点常量与请求头都在那），客户端 `harness.web` 不知道任何厂商的存在。要接第二家，就是加一节，
    不是改客户端。
