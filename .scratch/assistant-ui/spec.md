...
6. **threadId 的归属由 03 先定、06 收终，结论写回本节**：谁铸 id、写在哪个对象上、下一轮 run 怎么带上它。
   ...
   **06 走了 `adapters.threadList`，归属易主，本段即终稿。** 03 记录的「agent 自持」到 04/05 为止仍然
   成立；06 起主人是 **`App` 的 React state**：

   - **铸造点**：`useState(() => crypto.randomUUID())`——与 `HttpAgent` 构造函数自铸的形状一致（实测
     默认即 UUID v4），但不再借用它：agent 构造出来当场被写上我们铸的 id。
   - **写在哪个对象上**：React state 是唯一主人；**agent 是唯一的回写目标**，且只在 `adoptThread`
     一处回写（新建与恢复都过它）。运行时仍只读 agent（`buildRunInput`），两层间没有第二个通道。
   - **两处如何保持同步**：`adoptThread` 在**任何 await 之前**同步写 agent + setState——adapter 文档的
     硬规则（先设选中的 id，再等历史；被后来切换取代的那次，其消息会被运行时丢弃，id 必须已经指着
     赢家）。除此之外没有任何地方写 `agent.threadId`。
   - **实测**：新建 → 新 id 新文件；恢复 → state、agent、头部横排、列表高亮四处同值；恢复后续聊
     追加进旧日志（15 800 → 28 127 字节），线上的 RunAgentInput 与 03 时逐字同形。
...
- 不做侧边栏、附件、语音、听写、反馈这些 assistant-ui 的附加能力——当前界面没有这些需求。
  抄进来的 thread 元素自带的附件、分支、复制这些槽位不主动接，但也不为了去掉它们而改上游代码。
  **这条 2026-09-15 收窄了一格**：侧边栏本身由 `.scratch/project-sidebar` 立项（项目分组、归档、
  设置都在那里），本特征仍然不做；会话列表这一版按对位还原成顶部横排，由那份特征搬进侧边栏
- 不借换库之机重新设计界面：自建面板的视觉与行为按对位还原。**项目目录面板不再属于本特征**
  （07 置 `wontfix`，被 `.scratch/project-sidebar` 取代，见那份 spec 的决策 7）
...
8. ~~项目目录面板的读、绑定、系统目录选择三条路可用。~~ **2026-09-15 划掉**：07 置 `wontfix`，
   被 `.scratch/project-sidebar` 取代。三条路由（`GET/POST /api/project`、`POST /api/project/pick`）
   那时已经落地并通过验收，形状与行为契约在 `project-sidebar` 05 里继续用。
...
- **03 是全特征唯一的高风险票**：新库 + threadId 未知两件事压在同一票上。它落地后页面暂时只剩聊天，
  旧的审批门、reasoning、会话面板、项目面板的 TS 版本随 CopilotKit 的装配一起退场，由 04–06 接回来
  （原本写的是 04–07；07 作废后，那块面板的旧实现改由 `project-sidebar` 04 连同顶部会话横排一并清掉）。
...
- **`adapters.threadList` 是 experimental**：06 压在它上面，文档自己写着「可能在不通知的情况下更改」。
  ...
  **06 落地后的结论：用了它**（多线程来回切正是它的形状；`adapters.history` 是单线程页载恢复的形状，
  ...
  `{messages, state?, unstable_resume?}`，core 的类型是 `void`）——升级时若编译不过，先看这个联合
  是否又被改了。另外 06 给它的 `threadId` 字段每次变化都会让运行时重建主线程核（`invalidateThreadRuntime`），
  所以 id state 必须只在真切换时变，不能跟着渲染抖动。
...
- **05 落地后给 06 留的接点**：park 的中断只活在客户端内存，刷新即失，而服务端的 park 记录还在进程里；
  ...
  **06 落地后的实测结论：审批状态不随恢复回来，且不是客户端能补的。** 链条是：中断只存在于
  `RUN_FINISHED.outcome` 帧里，服务端重建（`frames.clj` 的 `apply-frames`）忽略该帧，重建出的消息不带
  `metadata.custom.agui.interrupts`；`fromAgUiMessages` 的读回逻辑因此空转。把它折进重建是 JVM 侧改动，
  被非目标「JVM 侧一行不动」挡住。实测后果（`t06-02`）：parked write 的卡显示 `Needs approval`
  （由无结果 part 推导），但无审批卡、composer 不 hold；恢复后发送会被两道真防线接住——本地自动取消
  该调用（卡翻 Failed），且 `@ag-ui/client` 拒发（agent 层记着 park："Thread has 1 pending interrupt(s)
  not addressed by resume: <id>"）。没有任何假续聊，但「恢复即无法再批」。若将来要修，改的是
  `apply-frames` 折 RUN_FINISHED 一处，客户端零改动即可全亮。
