...
同样地，**02 落地的四张表今天没有任何写入者**：`read`（04）才发行锚点，`replace`（05）才推进它们，
`undo_last_replace`（08）才读撤销记录。所以库里的 `hashline_*` 现在是空的、且**没有代码会让它非空**。
这不是缺陷，是顺序：存储先立、工具后长。但它有一个可观察的后果要记住——`write` 的「释放该文件全部
锚点」（07）实现之前，一次 write 不会清掉任何东西，因为那时还没有东西可清。
