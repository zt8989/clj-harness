...
- **模式与工具有自省面。** 模型可以问本会话在哪个模式（`harness.memory/editing-mode`），prompt.md 的
...
| `pi.setActiveTools` / `tool_call` 钩子 | 编辑模式解析器决定服务哪张表 + `write` 体内的守卫 | 本仓没有扩展 API；钩子引擎（hook-engine）尚未落地，不为它预留 |
...
- 不实现 hook-engine 的钩子点，`write` 守卫直接长在工具体内，将来挪进钩子点是一步机械搬迁。
...
全部未开始。**01–11 是 opt-in 落地**（`:editing {:mode :hashline}` 才生效），既有 189 tests / 930
assertions 一条不动；**12 翻默认**，那时才改写既有断言。**存储那一票（02）另有一条跨特征阻塞边：
`project-sidebar` 01 先落地**（它建库与迁移链），本特征只是往那条链上再加三张表。

| # | 票 | 阻塞于 |
| --- | --- | --- |
| 01 | 编辑模式落到配置：一个可读、可覆盖、可自省的开关 | — |
| 02 | 锚点本身：算法、归属、以及能跨重启活下来的存储 | 01, **project-sidebar 01** |
| 03 | 模式决定服务哪张工具表，以及被拒的名字怎么解释自己 | 01 |
| 04 | `read` 带锚点出行：看得见、可翻页、非文本点名拒绝 | 02, 03 |
| 05 | `replace`：按锚点改一段，改完就把下一笔要用的锚点交回手里 | 02, 03, 04 |
| 06 | 编辑被拒时，把「重试所需的一切」一起交回去 | 05 |
| 07 | `write` 是锚点的边界：释放、清撤销、拒绝回显、以及改完随手能接着改 | 02, 03, 04 |
| 08 | `undo_last_replace`：把文件**和锚点**一起退回前一态 | 05, 06 |
| 09 | 同一回合的多笔编辑：合成一次提交，或者一次都不提交 | 05, 07, 08 |
| 10 | `insert`：插在哪一行前后，锚点不动 | 05, 06 |
| 11 | `anchor_grep`：搜出来的命中行可以直接改 | 04 |
| 12 | 翻默认：hashline 成为默认实现，prompt 与文档跟上，既有断言重写 | 03–11 全部 |
