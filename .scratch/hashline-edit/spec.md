...
那时才改写既有断言（届时只动编辑工具那几个名字）。当前 **380 tests / 8468 assertions 全绿**，断言数
...
### ⚠ 开发中期的不完整状态（05 之后、06–11 之前）

**05 之前**，`:mode :hashline` 会拿走 `edit` 而给不出任何替代，是 12 绝不能翻默认的第一道闸门；`replace`
落地后这条已经不成立，**闭环已经通了**（read → replace → 拿答复里的锚点接着 replace，有用例钉住）。
仍然不完整的是剩下的四件事，按对模型的实际影响排序：

| 缺什么 | 今天的表现 | 归谁 |
| --- | --- | --- |
| 写盘边界 | `write` 之后旧锚点仍在库里算「已拥有」，池里少掉一批名字 | 07 |
| 撤销 | 库里记着撤销记录，但**没有任何工具能读它** | 08 |
| 拒绝的可操作性 | 漂移/未读/未展示一律是「去 read」，没有把重试所需的锚点一起交回 | 06 |
...
**给 06 的地基**：本票只做「输入合法且一切正常」的路径。`verify!` 的两种拒绝（漂移 / 未展示）与
`unresolvable!` 的两种拒绝（不是锚点 / 不属于本会话）现在都是**一句指名的话**，06 要做的是把它们变成
**可操作的回答**（把重试所需的当前锚点一起交回去）——所以那四个 `:reason` 现在就已经分好了，06 是往
上面加东西，不是重写。
