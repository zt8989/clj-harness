...
//
//   1. A FRESH PAGE, A RUN, AND A MODEL CHANGED BY HAND. The page mints its own id and
//      registers it with the server at the first send (the merge's amendment -- it used to
//      ask the server for an id at load), sends a turn, and the composer's per-session
//      model is switched through the picker -- the thing the acceptance list wants changed
//      "once in a round".
...
// THE MERGE CHANGED THIS STEP: a fresh page used to ASK the server for a session when its
// first listing landed, so this waited for that request. The page now MINTS its id in the
// browser and writes nothing (点击新增不立刻会话，发送才新建), registering it just before the
// first run -- so there is no request here to wait for, and what the id is read from is
// the page's own memory. The rest of this walkthrough is unchanged: the run below is the
// first send, and it is what puts the session in the home.
let id = null;
...
if ((await remembered(one)) !== id) fail("the reload did not come back to the session the page minted");
