...
**2026-09-21 修正后重跑**（把 `main` 合进 `brand-header` 时改的这一格）：下面第一、二条是合并后的
契约——**id 由页面铸、登记挪到第一次发送之前**——第三到五条与当天一致。

- **id 是页面自己铸的，发送之前一个请求都不发**：打开一个新页面，localStorage 里立刻有 id，
  而 `POST /api/sessions` 一次都没有（脚本数的那行是 `before anything was sent: 0`）。原来这一步
  恰好相反（空手 `POST /api/sessions {}`，服务端答一个 id）；它与侧栏那条「点击新增不立刻会话，
  发送才新建」不能共存，两边都留的代价就是登记换时刻。
- **第一次发送先登记、再发 run**：第一轮的请求顺序就是 `["sessions","agent"]`——登记带着**页面
  自己铸的那个 id**（`{"threadId":"8e60743f-771f-4aaf-b20a-180ce4913289"}`，这条路由允许客户端带
  id 且幂等），紧接着才是那一轮的 run。不认识的 id 仍然被点名拒绝，所以这个顺序是必须的：
  run 必须落在「这个家已经被要求保留它」之后。
- **每一轮的 body 都只有这一轮的动静**（三轮逐字相同，只有 append 里的那条消息不同；下面这段是
  当天那次的样子，合并后 `threadId` 同样是页面自己铸的 id，其余逐字不变）：
...
- **刷新回到同一个会话**：reload 之后 localStorage 里还是同一个 id，页面把两轮的对话重建出来，
  而且**没有再登记一次**（脚本数的就是这个：整个走查只出现一次 `POST /api/sessions`，
  就是第一轮那次）。
...
票 03 改的是**线上格式**：客户端不再把整段对话发回来；新会话的 id 由页面铸，但**必须先让这个家
认识它**（合并时把「谁铸 id」改成这样，见上面第一条）。这两件事在
