...
/// WINDOWS: a backslash in a CSS attribute value has to be escaped (`\`), and this run's
/// temp path is a Windows one -- without this the row is never found and every cell below
/// times out. The macOS path the original file was written against has none of these.
const pathSel = (p) => p.replace(/\\/g, "\\\\");
await clickRefresh();
await page.waitForSelector(`[data-slot="sidebar-project"][data-path="${pathSel(projectPath)}"]`, { timeout: 15000 });
const projectName = await page.$eval(
  `[data-slot="sidebar-project"][data-path="${pathSel(projectPath)}"] [data-slot="sidebar-project-name"]`,
...
await page.click(
  `[data-slot="sidebar-project"][data-path="${pathSel(projectPath)}"] [data-slot="sidebar-project-new-session"]`,
);
...
const projectGeometry = await page.evaluate(
  ...
  { projectPath: pathSel(projectPath), id: inProjectId },
);
check(
