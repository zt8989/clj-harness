...
- **三层**（从下往上装，每层可以盖住上一层的同名定义）：
  1. **基础层**：逐个工具勾选（`read` / `write` / `bash` …），加这一层的基础 hooks。
...
14. **基础 hooks 层是"覆盖"，不是"重写"。** `:base :hooks {:off [...] :on [...]}`：**默认每个声明的 hook
    每个 agent 都跑**（今天的行为），把一个名字写进 `:off` 才替这个 agent 拿掉它。要能按名点一条 hooks.edn
    里的声明，声明多一个**可选** `:name`（kernel 的 built-in 行本来就叫 `:name`）；没写名字的仍按今天
    的位置 id（`stop#0`）寻址。**复合层技能带来的 hooks 不走这里**，由复合层替这个 agent 装上（见 12）。
