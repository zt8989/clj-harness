...
12. **复合层最后装，它就是"最后提供工具"那一层。** 技能与 MCP 的工具在基础层与增强层之后进入表；
    ...
    - **注入的规矩不变**：技能正文排在提问**之后**（`context-frames` 从 7 起的读法），
      ...
      `:skills :load` 只决定哪些技能**在发送时被自动注入**，不改这条位置规矩；注入本身走**唯一那个口**
      （决策 16），不另起第二条路。
...
14. **基础 hooks 层管两类行：kernel 自己的内建行，与 `hooks.edn` 的声明。** `:base :hooks {:off [...]
    ...
    - **内建行**里第一条是**指令文件（AGENTS.md）的注入**（落点是**唯一那个注入口**，决策 16）。那个"会话出生时折进一段 `<instructions>`"的动作
...
16. **上下文注入只有一个出口，而它不在工具表里。** 指令文件、技能清单与技能正文、作业结束的通知、
    以及 hook 想放进对话的东西——**全部**走同一个**内建、对 agent 不可见**的注入口，而**这个口今天就有**：
    `harness.kernel.loop` 里"每次模型调用之前把历史装饰一遍"的那一步（`:before-llm`），它加进去的每一条
    都发一条 `:context.injected`（客户端那张 CUSTOM 卡就是它）——loop 的 docstring 自己写着
    "This remains the ONE place they can enter the conversation"。edge 今天喂它的是
    `cap/project.clj` 的 `before-llm`（`cap/skills.clj` 的 `derived-injections` ＋ `cap/jobs.clj` 的通知）；
    `.scratch/action-fusion` 票 07 把**同一个口**以 `context/inject!` 露给沙箱与 hook。
    三层配置决定的只是**注入什么、什么时候**，不是**用哪条路**——今天各写一遍的那三条归到这**一个**口上，
    **不许再多一条**。开场（指令文件 ＋ 技能清单）是**同一个口**的另一种时机：在**会话出生**时折进去，
    不走 `:before-llm`，但加进对话的那条消息形状一致。
    - **它没有格。** 它不是工具表里的一条、不在 `specs` 里、agent 看不见它——所以三层的勾选里**没有它**；
      能勾的是"注不注、注哪些"（基础层的指令文件、复合层的技能、`UserPromptSubmit` 的声明）。
    - **时机各是各的，口是一个**：开场在**会话出生**时、`/name` 与 `UserPromptSubmit` 在**那一轮之前**、
      作业通知在**下一轮之前**——口只管"加一条消息进这场对话"，"出生时折一次"那条因此不受影响。
...
`.scratch/action-fusion/spec.md`（复议 3，2026-09-26）与本特性在同一个地方相交：**一次调用能带什么、
能被谁看见**。两处接口，写在这里免得两张 spec 各说一套：

- **注入口**：本特性的三层只往**唯一那个内建注入口**里放东西（决策 16）；action-fusion 票 07
  （`context/inject!`，沙箱与 hook 都能用的那个）就是它。**两张票不许各造一个口**——票 07 落的是
"沙箱/hook 也能用它"，07 之后的 `context/inject!` 应当就是本特性与既有三条注入路共用的那一个实现。
- **可见层**：action-fusion 票 06 给每个工具一个**不上 wire**的 `:visibility`（`:all` / `:main` /
  `:hidden`），`eval` 与委派工具是 `:main`，**`forbidden` 集合退场**。若它先落，本特性的对应处按它改：
  - 决策 9（`eval` 与委派工具永不进任何子 agent 的表）＝"`:visibility :main` 的名字不进子 agent"，
    那套 `forbidden` 换成读它；
  - 委派工具的名字从 `agent` 变成 `sub_agent`——本 spec 里写 `agent` 与 `forbidden` 的两处跟着改。

  `:visibility` 与三层的 `:base :tools` **不是一回事**：后者是"**这个 agent** 的配置"，前者是
  "**工具自己**声明谁能看见它"。两个都算进 `:served`（都满足才在），谁都不许悄悄盖过谁。
