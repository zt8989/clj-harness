...
- **基础层**：一行一个能力（`bash` / `read` / `write` / `glob` / `grep` / `web_fetch` / `web_search` /
  ...
  下面再一段是**基础 hooks**：**内建行**（第一条是**指令文件 / AGENTS.md 的注入**）与 `hooks.edn` 的每一条
  声明各一行，开关就是 `:base :hooks :off` 里的成员（内建行按名字，声明按 `:name` 或位置 id）。
...
- [ ] 基础 hooks 段里看得到**指令文件注入**那一行（内建行）；关掉它之后，这个 agent 的开场真的没有
      `<instructions>` 块（与 04 的同一条断言各跑一次，页面不自己再算一份判断）。
