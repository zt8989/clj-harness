...
## 票 03 剩余半落地：文本按快照写（2026-09-26）

推理族那半早就落了（ADR 0009）。**这一族的口径与它不同**——文本没有模型行兜着，被掐断的 run 只有记录这
一个去处——所以票面要的「合并成快照」落的不是「攒几个 delta 一起写」，而是**整句一行**：

- **记录里的文本是「到此刻为止的全文」**，每 `http/text-snapshot-ms`（默认 75）落一行，形状是
  `CUSTOM text/snapshot`；**没有**去改厂商 payload 的形状（`row-of` 那条逐字节的规矩不动）。
- **第一条立即落**，然后才按周期；**消息的 END 与 run 的终点都先补一次**。第一句立即落是必需的：只按
  周期的话，一条在第一个 tick 之内被掐断的 run 记录里一个字都没有——`http_test` 那条**不喂终点帧**的
  既有用例正是这么量到的（它原来靠「逐条写」通过，这一版先红，改了规则之后绿）。
- **折叠按「替换」读**，没有这条消息时**凭它建一条**：只有一行的读者（窗口开在句子中间、日志头被截）
  手里也是整句。
- **`replay/wire-custom-names` 里加了 `text/snapshot`**：那个集合的注释本来就写着「新族必须在同一个提交里
  加进来」。不加就会把快照读成关于 harness 自己的事实，**静静地把正文丢掉**（实测：折出来是空字符串）。
- **线上一个字节没动**：两个 sink 照旧广播每一帧、照旧收进 `settle!`，客户端照旧一个 token 一帧；改的是
  哪些**行**存在。两条路线共用 `http/text-lines`（与 `reasoning-frame?` 同形）。
- 子 agent 的快照带上它所代表的那一帧的 `:seq`（记录/总线那道边界靠它比对）。

**判据**：`replay-test` 35 用例 / 194 断言全绿（新增一条：同一场 run 两种写法折出同一场对话，且「只有
一行」的读者也拿到整句）；`http-test` 112 / 1222 全绿（新增三条：记录里没有逐字帧而有一条整句快照；
周期的两端；那条不喂终点帧的用例改了 kinds 期望并加了「半句仍在」一句）。ADR 是 **0010**
（`docs/adr/0010-the-running-answer-is-written-as-snapshots.md`）。**没动的**：工具调用、状态、消息的落法
（关键事件照旧逐条），以及老日志的读法（delta 那一支一个字没改）。
