...
**先用 `node dev.mjs`。** 它把上面那句「先确认 8080 是空的」整条顾虑去掉了：后端在 **0 号端口**上绑
（OS 分配），脚本读后端自己报出来的那个端口当反代目标，前端随机端口起——源码里没有端口号，
也不再需要谁先让出 8080。走查只需要：

```bash
node dev.mjs --scripted /path/to/script.json --ui-port 5211
...
**手动起法**（不想用这个脚本，或者要看后端自己的输出时）——临时家目录与脚本文件
按下面这几条命令现建（**这一次跑完后已删掉**，本仓的规矩是临时目录不留）：
