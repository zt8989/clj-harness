...
2. **开关照左侧那一对的做法，落在右边。** `components/sidebar-toggle.tsx` 今天是一件契约的两个地方：
   ...
   所以那一对也都不画——一条点了没反应的开关比没有开关更坏。~~ **2026-09-25 翻掉**：`md` 以下右栏是**抽屉**了
   （浮在对话之上、页面那层遮罩、点遮罩或栏头前缘那颗收起关掉，与左栏在 `lg` 以下同形），两条开关因此每个宽度都画。
   那条「不许把主对话压到不能用」没变，只是今天的答案是把 26rem 浮起来而不是整列不画；见本文件末尾 2026-09-25 那条。
...
- ~~**不动窄窗口下右栏不画的决定**~~（**2026-09-25 翻掉**：`md` 以下改成抽屉——理由与验收见本文件末尾那一条），
  不动镜像里没有 composer 的决定。
...
### 2026-09-25：`md` 以下不再是「不画」，是抽屉（主人的话：「右边的折叠和左边一样显示浮动图标」）

**同一列，两个形状，`md` 分界。** 从 `md` 往上是原来那一列并排（一个字没改：第三个 `shrink-0` 孩子、`w-[26rem]`、
主对话让出正好这么多）；`md` 以下是**左栏在 `lg` 以下那个形状的右版**——这一列 `absolute inset-y-0 end-0 z-30` 浮在
对话之上，身后是页面画的那层 `z-20` 遮罩（`app.tsx` 的 `data-slot="right-pane-backdrop"`，`md:hidden`），点遮罩或栏头前缘那颗
收起关掉。`max-w-[calc(100%_-_3rem)]` 故意留一条 48px 的边：铺满就没遮罩可点，而手机上没有 Escape 可按。

- **右上角那颗打开图标每个宽度都画**（去掉 `hidden md:flex`）：`md` 以下它是抽屉那扇门，`md` 以上它仍是唯一的门
  （右栏这一侧没有 rail）。原来「一条点了没反应的开关比没有更坏」那条理由**没变**，变的是它下面的事实——栏不再是
  `display: none`，它是个抽屉。
- **同一台手机上左右两个抽屉一次只开一个**：判据是 `rightPaneIsDrawer()`（读 `md`，与那一列自己的 `md:` 类串同一个
  断点；`matchMedia` 当场判、不挂 resize 监听，照 `sidebar-toggle` 那条规矩），开右栏收起左栏、展开左栏关右栏——
  否则两层 30% 遮罩叠在同一屏上。`md` 以上它答 `false`，两扇门互不干涉（左栏那时可能仍是抽屉，但右栏是并排的邻居）。
- **Escape 先关最上面那个抽屉**：`md` 以下右栏开着时 Escape 关右栏而不是左栏；`md` 以上它什么都不做，与左栏同一条
  理由（并排的一列不是盖在谁身上的东西）。
- **取数那三条停法一个字没改**（卸载 / 页面隐藏 / 栏不在画面上）。

**当场量的（真浏览器，`node scripts/dev.mjs --scripted`）：**

| 量的是什么 | 390×844 | 900×800 |
|---|---|---|
| 右上角那颗打开图标 | `x=350 y=8 32×32 absolute z-40 flex` | 同左 |
| 右栏 | `x=48 w=342 h=844 absolute z-30`，`max-width: calc(100% - 48px)` | `x=484 w=416 position: static` |
| 栏后遮罩 | `390×844 display:block z-20` | `display:none` |
| 左栏抽屉（右栏开着时） | `display:none`（已被收起） | 互不干涉 |
| 点遮罩 / 点栏头那颗收起 / Escape | 三条都关掉，浮动图标回来 | — |
| 栏开着 3 秒的 `/jobs`+`/subagents` | 8 次（每秒一对）；点收起后只剩当场那一对在飞的读，**再无一条** | — |
| 整页 `scrollWidth` | 390 = 视口（不横滚） | — |

截图在 `.scratch/right-pane-tasks/evidence/`：`v06-01-right-drawer-390.png`（抽屉开着、遮罩那条 48px 的边看得见）、
`v06-02-right-corner-open-390.png`（关着时右上角那一颗）、`v06-03-right-column-1280.png`（1280 并排的一列）。

套件侧：`ui/test/suites/right-pane.tsx` 那条「`md` 以下两颗都不画」改成「每个宽度都画」，并新增三条源码读（类串既有
`absolute` 又有 `md:static`、遮罩是页面画的且 `md:hidden`、两扇门都走 `rightPaneIsDrawer`）；`CONTEXT.md` 的「右栏」词条与
`docs/architecture/client.md` 那两行同改。`npm test` 144/144、`tsc --noEmit` 干净、`npm run build` 过。

**没做的（留给主人定）：** 没给窄窗下的抽屉加动画或边缘手势（点遮罩与那颗收起就够，左栏今天也没有）；没把 `md` 与
`lg` 两个断点收成一个（右栏在 768、左栏在 1024，各有各的理由：对话要 768 才放得下两列，侧栏要 1024 才够宽）。
