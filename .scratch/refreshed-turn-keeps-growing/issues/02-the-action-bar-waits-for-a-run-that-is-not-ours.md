...
4. **而且只摆在「正在被写的那一轮」的末尾**（主人第三次报：发下一条消息的瞬间，**每个** turn 末尾各长一颗点）。
   `writing` 说的是**对话**（有人在回答它），而每个 turn 末尾都画这一格——两件事必须分开。
5. **落地时 `turn/*` 也到了，于是这一格的判据搬到了那一轮自己身上**（主人拍的方向）。三个事实各有各的家
   （`lib/live-turn.ts` 的 `wearsWorkingDot`）：`turn.open`（那一轮自己的生命周期，`turn/start` / `turn/end`；
   一个 turn 比一次 run 大——park 不关它）、`writing`（此刻有人正在写；parked 的轮还开着而没人写，那里的标记是卡）、
   `isLast`（这一格就是那一轮的末尾——同时只有一轮开着）。**刷新那一格靠窗口的种子**：fact 家族只服务「你看着的
   时候」（决策 5），落在一轮中间时它对这一轮一言不发，页面从窗口自己的词种下去。走查实测（两轮的脚本）：第二轮
   在流的那 3.6 秒里 `dots` **一直是 1**（`assistants` 已经是 2），第一轮末尾只有它自己的家具；另一次走查（刷新落在
   工具还在跑的时候）刷新之后仍是 **1**。
