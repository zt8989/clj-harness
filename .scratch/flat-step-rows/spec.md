...
1. **~~相邻两行的间距是 8px，不是票面预测的 12px。~~ 2026-09-21 推翻：是 0px 盒距（12px 全是行自己的
   padding），见文末复议。** 票面是按「同一个组里的兄弟节点」算的，
...
2. **决策 5「思考行把想法首行摊出来」照旧成立，但它只描述**已经想完**的那段。** ~~正在到达的思考
   ...
   所以「想 → 读 → 再想」会开合两次；~~ **2026-09-22 推翻：流式那段不展开**，它在行上滚、停下来才
   回到首行（理由与代价见文末复议与 `.scratch/thinking-row-tail/`）。判据仍取组的状态，「想 → 读 → 再想」
   于是换两次尾巴、再回到首行；历史会话永远是折的（这条没变）。本文件那张表里「点开后是整段」照旧成立：
   只有**打开着**的那扇窗在流式期间用上游的 256px 窗口与跟随，停下来之后整段摊开。
...
### 复议（2026-09-21）：相邻步骤的盒距是 **0**，那 12px 全是行自己的 padding；卡也照这一条

**「相邻两行的间距是 8px」被推翻**（上面「与票面的四处差异」第 1 条已就地划掉）。实测：8px 是
「消息组 `gap-y-6` 的 24px 减去中段 `-mt-4` 抵掉的 16px」的**余数**，而这个余数谁也不认识——
一行的 `py-1.5` 是 6px，两条相邻的步骤之间读到的空白是 12px，那个 8px 是**多出来的**。

更要紧的是它**不是唯一的数**：**注入的上下文卡一条负边距都没有**，所以卡与卡之间是整整 24px
（一个轮次的距离），工具行之间 8px，同一消息里的两行 0px——三个数并存在一栏里，就是主人报的
「工具调用和上下文注入 间距不一样」。2026-09-21 一次修掉：抵掉**整条**组间距（`-mt-6`，名字
`STEP_SPACING`），并且**卡的消息也吃这一条**（`isStepAfter`）——现在每一对相邻步骤都是 0px 盒距，
读到的 12px 只有行自己的 padding；人的消息那一侧照旧保留组间距，那是轮次边界该有的。

本票的现场、决策与量法（含这套走查原先量不到这件事的三处原因：夹具只画得出一张卡、`messageSpan === 1`
的过滤器丢掉了同消息的那几对、两张截图都拍在最后于是同哈希）在 `.scratch/step-row-gaps/spec.md`，
证据 `evidence/` 四张图 + 修前修后的原样输出。**决策 1 那句「相邻两行由各自的 `py-1.5` 隔开（12px）」
现在才是字面成立的**——修之前它是 8px，而票面那句话当时是对的、实现不是。

### 复议（2026-09-22）：思考行不自己展开了；流式那段在**行上**滚，停下才回到首行

**上面 2026-09-17 那段复议的第 2 条被推翻一半**（已就地划掉那半段，这里说现在是什么）：那段思考**不再**
自己展开，也不再用上游那扇窗滚动显示——**行上**那一行字自己滚：流式期间行上装的是**已经到达的那一段**
（压成一行），整行被一只 `overflow: hidden` 的窗**往左拖**到它的末尾停在窗的右边缘（`transform` +
transition，所以是**滑**不是跳），字从**左边出去**、新字从**右边进来**；最后一个 token 落下，行上回到
**首行**——决策 5 一字未动，它现在描述的是两种状态。

**没有展开的代价，也就没有了那扇窗的默认开销**：折着的那扇抽屉 Radix 不渲染子节点（实测高 0px、无文本），
所以流式里真正在动的只有这一行，行高全程 28px（一行 13px 字 + `py-1.5`）——**下面那些步骤不会被推走**，
这是当初「自己展开」最贵的那个代价。**窗口、渐隐、跟随都还在**，只给**点开它的人**（`streaming && open`）。

判据三条、五条决策与代价、真浏览器 21 条 GREEN 的现场与四张截图：**`.scratch/thinking-row-tail/`**（`spec.md` +
`evidence/README.md`；那天的**第一版**用裁左边缘的布局做同一件事、被主人一句话推翻——「从左边字退出，
右边字插入」说的是运动，而布局位移没有中间态；复议与它换来的判据都在那份 `spec.md` 里）。文字规则本身搬去了
`lib/reasoning-preview.ts`，UI 套件直接测（`suites/reasoning-row.ts`，`EXPECTED_CASES` 88 → 91）——那个文件
原来把三条规则关在 `message-parts.tsx` 里，套件一条也够不着。
