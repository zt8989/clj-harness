...
自己展开，也不再用上游那扇窗滚动显示——**行上**那一行字自己滚：流式期间行上的摘要换成「已到达内容的
最后 120 字」，而这一窗**从左边缘裁**（`direction: rtl` 的盒子 + 内层 `ltr`），最新到达的字留在右边、
旧的从左边跑出去；最后一个 token 落下，行上回到**首行**——决策 5 一字未动，它现在描述的是两种状态。
...
判据三条、四条决策与代价、真浏览器 20 条 GREEN 的现场与四张截图：**`.scratch/thinking-row-tail/`**（`spec.md` +
`evidence/README.md`）。文字规则本身搬去了 `lib/reasoning-preview.ts`，UI 套件直接测（`suites/reasoning-row.ts`，
`EXPECTED_CASES` 88 → 91）——那个文件原来把三条规则关在 `message-parts.tsx` 里，套件一条也够不着。
